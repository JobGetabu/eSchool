    ...
    public partial class FeeCharts : UserControl
    {
        ...
        private async void UpdateChart(int year)
        {
            ...
            using (var context = new EschoolEntities())
            {
                ...
                var incListAsync = await Task.Factory.StartNew(() =>
                {
                    return context.Incomes.Where(x => x.Year == year)
                    .ToList();
                });
            ...
            var incs = new ChartValues<decimal> { incJan, incFeb, incMar, incApril, incMay, incJune, incJuly, incAug, incSep, incOct, incNov, incDec };

            var exps = new ChartValues<decimal> { expJan, expFeb, expMar, expApril, expMay, expJune, expJuly, expAug, expSep, expOct, expNov, expDec };

            var feeps = new ChartValues<decimal> { janFee, febFee, marFee, aprilFee, mayFee, juneFee, julyFee, augFee, sepFee, octFee, novFee, decFee };

            cIncome.Clear();
            cIncome.AddRange(incs);
            cExpenses.Clear();
            cExpenses.AddRange(exps);
            cFeePaid.Clear();
            cFeePaid.AddRange(feeps);
